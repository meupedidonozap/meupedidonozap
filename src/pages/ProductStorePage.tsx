...
import { getStoreBySlug, getCategoriesByStore, getProductsByStore, getCouponsByStore } from '@/data/mockData';
...
export default function ProductStorePage() {
  ...
  const coupons = store ? getCouponsByStore(store.id) : [];
                          ...
                          <div className="flex gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="mt-1 text-sm text-muted-foreground">
                                Subtotal: {item.quantity} x {formatCurrency(item.price)} ={' '}
          ...
          {selectedProduct && (
            <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {uniqueColors.map(color => (
                      <Button
                        ...
                        size="sm"
                        onClick={() => setSelectedVariant(prev => ({ ...prev, color, size: undefined }))}
                      >
                        {color}
                      </Button>
                    ))}
              ...
              <Button
                className="w-full"
                disabled={
                  (uniqueColors.length > 0 && !selectedVariant?.color) ||
                  (availableSizes.length > 0 && !selectedVariant?.size)
                }
