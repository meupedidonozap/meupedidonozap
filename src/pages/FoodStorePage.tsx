...
import { getStoreBySlug, getCategoriesByStore, getFoodItemsByStore } from '@/data/mockData';
...
export default function FoodStorePage() {
  ...
  const allFoodItems = store ? getFoodItemsByStore(store.id) : [];
  ...
  const getItemQuantity = (itemId: string) => {
    const cartItem = cart.items.find(i => i.productId === itemId);
                        ...
                        <CardContent className="flex gap-4 p-4">
                          <div className="flex-1">
                            ...
                            <p className="mt-2 font-bold text-accent">
                              {item.price > 0 ? formatCurrency(item.price) : 'A partir de'}
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 border-t bg-card">
        <div className="container flex h-16 items-center justify-around">
          ...
          >
            ...
            {totalItems > 0 && (
              <Badge className="absolute -right-2 -top-1 h-5 w-5 rounded-full bg-accent p-0 text-xs">
                {totalItems}
              </Badge>
            )}
        ...
        <div className="fixed bottom-20 left-0 right-0 z-40 p-4">
          ...
          >
            <Link to={`/${store.slug}/checkout`}>
              <ShoppingCart className="h-5 w-5" />
              Ver Carrinho ({totalItems} itens) • {formatCurrency(cart.total)}
            </Link>
          </Button>
        </div>
      )}
